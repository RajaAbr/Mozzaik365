...
export const MemeCommentList = ({ meme }: MemeCommentListProps) => {
  ...
  const { isLoading, data: comments } = useQuery(
    ["memeComments", meme.id], // Dynamic query key
    () => fetchPost(postId as number), // Query function
    {
      enabled: !!postId, // Fetch only when postId is not null or 0
  ...
  const { isLoading, data: comments } = useQuery({
    ...
    {
      enabled: meme.id == openedCommentSection,
