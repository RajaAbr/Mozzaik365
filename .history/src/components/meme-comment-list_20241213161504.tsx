...
export const MemeCommentList = ({ meme }: MemeCommentListProps) => {
  ...
  // Mutation of comment create
  const { mutate } = useMutation({
    mutationFn: async (data: { memeId: string; content: string }) => {
      await createMemeComment(token, data.memeId, data.content);
    },
    onSuccess: () => {
      // Refresh meme comment list on Create meme comment success
      queryClient.invalidateQueries(["memeComments"]);
    },
  });

  // Query to fetch meme comment list
  const { isLoading, data: comments } = useQuery({
    ...
    queryFn: async () => {
      ...
      // Add comment author data to memeComment
    ...
    // For more performance, fetch meme comments on collapse opening
    enabled: openedCommentSection != null,
  });
  ...
  const handleSubmit = (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (commentContent) {
      mutate(
        ...
        {
          onSuccess: () => {
            setCommentContent(""); // Clear the input after adding
          },
        }
      );
    }
      ...
      <Collapse in={openedCommentSection === meme.id} animateOpacity>
        <Box mb={6}>
          <form
            ...
            data-testid={`meme-add-comment-form-${meme.id}`}
          >
            <Flex alignItems="center">
              <Avatar
                borderWidth="1px"
              ...
              <Input
                ...
                variant="outline"
                data-testid={`meme-new-comment-input-${meme.id}`}
              />
            </Flex>
          </form>
        </Box>
