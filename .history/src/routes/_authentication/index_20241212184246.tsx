...
export const MemeFeedPage: React.FC = () => {
  ...
  const fetchMemes = async ({
    pageParam = 1,
  }): Promise<{ data: MemeWithAuthor[]; nextPage: number | null }> => {
    ...
    const nextPage = data.length > 0 ? pageParam + 1 : null; // Determine if there's more data
    return { data, nextPage };
  };

  const { isLoading, data: memes }: any = useQuery<any>({
