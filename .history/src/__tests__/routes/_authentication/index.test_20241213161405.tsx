  ...
  describe("MemeFeedPage", () => {
    ...
    it("should fetch the memes and display them with their comments", async () => {
      ...
      await waitFor(() => {
        ...
        // Simulate click on comment overflay component to display comments
        const linkOverlayElement = screen.getByTestId(
          "meme-comments-section-dummy_meme_id_1"
        );
        expect(linkOverlayElement).toBeInTheDocument();
        fireEvent.click(linkOverlayElement);
    ...
    /*it("should create new comment and display it", async () => {
      renderMemeFeedPage();

      // Simulate click on comment overflay component to display comments and add comment input
      await waitFor(() => {
        const linkOverlayElement = screen.getByTestId(
          "meme-comments-section-dummy_meme_id_1"
        );
      ...
      // Simulate change the input and submit the add comment form
      await waitFor(() => {
        const newCommentInput = screen.getByTestId(
          "meme-new-comment-input-dummy_meme_id_1"
        );
        const newCommentForm = screen.getByTestId(
          "meme-add-comment-form-dummy_meme_id_1"
        );
        fireEvent.change(newCommentInput, { target: { value: "NewComment" } });
        fireEvent.submit(newCommentForm);
      });
      // Check if the new comment is displayed in the comment list
      await waitFor(() => {
        const newCommentElement = screen.getByTestId(
